...
const C = {
  ...
  border: '#2A2A2A', gold: '#FFD700', goldDim: '#C9A800',
  white: '#FFFFFF', gray: '#B0B0B0', grayDim: '#555555',
...
const BRANCH_INFO: Record<string, { address: string; phone: string; waze: string }> = {
  '8fed141d-0e7c-46c1-803b-88d3d811c1f8': {
    ...
    phone: '058-5505014', waze: 'https://waze.com/ul/hsv9hu47xz',
  },
  '3ab15ad1-e835-492b-bae5-11b202ee2314': {
    address: 'אזור תעשיה שער בנימין, מעבר מכמש',
    phone: '058-5505012', waze: 'https://waze.com/ul/hsv9j62u44',
  },
}
...
export default function Home() {
  ...
  useEffect(() => {
    if (!isOpen) {
      const t = setTimeout(() => setShowClosedPopup(true), 3000)
    ...
    if (saved) {
      try {
        ...
        if (s.orderId && s.expires > Date.now()) {
          ...
          // שחזר פירוט הזמנה מ-localStorage
  ...
  async function restoreBranch(branch: Branch, savedScreen: Screen) {
    ...
    setCategories(cats); setMenuItems(items); setToppings(topRes.data || [])
    if (cats.length > 0) setActiveCategory(cats[0].id)
    setLoading(false); setScreen(savedScreen)
  }
  ...
  function openItem(item: MenuItem) {
    ...
    setSheetPaidAddons([]); setSheetNoLettuce(false); setSheetNotes('')
    setSheetQty(1); setSheetSetDrink(''); setSheetSetAddon(''); setShowBottomSheet(true)
  }

  function openEditItem(index: number) {
    const c = cart[index]
    setSelectedItem(c.item); setSheetSauces(c.sauces); setSheetSalads(c.salads)
    setSheetPaidAddons(c.paidAddons); setSheetNoLettuce(c.noLettuce)
    setSheetNotes(c.notes); setSheetQty(c.quantity)
    setSheetSetDrink(c.setDrink || ''); setSheetSetAddon(c.setAddon || '')
    setEditCartIndex(index); setShowCart(false); setShowBottomSheet(true)
  }
  ...
  function addToCart() {
    ...
    const newItem: CartItem = {
      ...
      setDrink: sheetSetDrink || undefined, setDrinkExtra: drinkExtra || undefined,
      setAddon: sheetSetAddon || undefined, setAddonExtra: addonExtra || undefined,
    }
    if (editCartIndex !== null) {
      setCart(prev => prev.map((c, i) => i === editCartIndex ? newItem : c))
      setEditCartIndex(null); setShowBottomSheet(false)
    } else {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: C.bg, fontFamily: 'Heebo, sans-serif' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ color: C.gold, fontSize: 15, fontWeight: 600 }}>טוען...</div>
      </div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Heebo, sans-serif', direction: 'rtl' }}>
      ...
      {showInstallBanner && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 999, background: '#1A1A1A', borderTop: '1px solid #FFD700', padding: '14px 16px', direction: 'rtl', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={async () => { if (installPrompt) { installPrompt.prompt(); const r = await installPrompt.userChoice; if (r.outcome === 'accepted') setShowInstallBanner(false) } }}
            style={{ background: '#FFD700', color: '#000', border: 'none', borderRadius: 10, padding: '8px 16px', fontWeight: 800, fontSize: 13, cursor: 'pointer', fontFamily: 'Heebo, sans-serif', flexShrink: 0 }}>התקן</button>
          <button onClick={() => setShowInstallBanner(false)} style={{ background: 'none', border: 'none', color: '#6B7280', fontSize: 20, cursor: 'pointer', padding: '4px', flexShrink: 0 }}>✕</button>
        </div>
      )}
    </div>
  )
    ...
    return (
      <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Heebo, sans-serif', direction: 'rtl' }}>
        ...
        <div style={{ padding: '24px 16px', maxWidth: 480, margin: '0 auto' }}>
          ...
          {/* *** פירוט הזמנה *** */}
          {orderCart.length > 0 && (
            <div style={{ background: C.bgCard, borderRadius: 18, border: `1px solid ${C.border}`, marginBottom: 14, overflow: 'hidden' }}>
              <button
                onClick={() => setShowOrderDetails(prev => !prev)}
                style={{ width: '100%', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>
                <div style={{ fontWeight: 800, fontSize: 15, color: C.white }}>📋 פירוט ההזמנה</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ color: C.gold, fontWeight: 800, fontSize: 15 }}>{fmt(orderFinalTotal)}</span>
                  <span style={{ color: C.gray, fontSize: 18 }}>{showOrderDetails ? '▲' : '▼'}</span>
                </div>
              </button>
              ...
              {showOrderDetails && (
                <div style={{ padding: '0 20px 20px', borderTop: `1px solid ${C.border}` }}>
                  {orderCart.map((c, i) => (
                    <div key={i} style={{ paddingTop: 14, paddingBottom: 14, borderBottom: i < orderCart.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 700, fontSize: 15, color: C.white, marginBottom: 4 }}>
                            <span style={{ color: C.gold, fontWeight: 900 }}>{c.quantity}×</span> {c.item.name_he}
                          </div>
                          {c.sauces.length > 0 && <div style={{ fontSize: 12, color: C.gray, marginBottom: 2 }}>🧄 {c.sauces.join(', ')}</div>}
                          {c.salads.length > 0 && <div style={{ fontSize: 12, color: C.gray, marginBottom: 2 }}>🥗 {c.salads.join(', ')}</div>}
                          {c.paidAddons.length > 0 && <div style={{ fontSize: 12, color: C.gold, marginBottom: 2 }}>➕ {c.paidAddons.join(', ')}</div>}
                          {c.setDrink && <div style={{ fontSize: 12, color: '#60A5FA', marginBottom: 2 }}>🥤 {c.setDrink}{c.setDrinkExtra ? ` (+₪${c.setDrinkExtra})` : ' (כלול)'}</div>}
                          {c.setAddon && <div style={{ fontSize: 12, color: C.gold, marginBottom: 2 }}>🍟 {c.setAddon}{c.setAddonExtra ? ` (+₪${c.setAddonExtra})` : ' (כלול)'}</div>}
                  ...
                  <div style={{ paddingTop: 14, borderTop: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ color: C.white, fontWeight: 900, fontSize: 17 }}>סה״כ לתשלום</div>
                      <div style={{ color: C.gray, fontSize: 12, marginTop: 2 }}>{payLabel}</div>
                    </div>
                    <div style={{ color: C.gold, fontWeight: 900, fontSize: 20 }}>{fmt(orderFinalTotal)}</div>
                  </div>
                </div>
              )}
            </div>
  ...
  if (screen === 'order') return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Heebo, sans-serif', direction: 'rtl' }}>
      <div style={{ background: C.bgCard, borderBottom: `1px solid ${C.border}`, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => setScreen('menu')} style={{ background: C.border, border: 'none', borderRadius: 10, padding: '8px 10px', cursor: 'pointer', fontSize: 18, lineHeight: 1, color: C.white }}>→</button>
      ...
      <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
        ...
        <div style={{ background: C.bgCard, borderRadius: 18, padding: 20, marginBottom: 14, border: `1px solid ${C.border}` }}>
          ...
          <input type="tel" value={orderPhone} onChange={e => setOrderPhone(e.target.value)} placeholder="05X-XXXXXXX" dir="ltr"
  ...
  for (const cat of categories) itemsByCategory[cat.id] = menuItems.filter(m => m.category_id === cat.id)
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Heebo, sans-serif', direction: 'rtl', paddingBottom: cartCount > 0 ? 96 : 0 }}>
        ...
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <div style={{ padding: '12px 16px 10px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
              ...
              <div>
                <div style={{ fontWeight: 900, fontSize: 16, color: C.white }}>פלאפל בתחנה</div>
          ...
          <div style={{ display: 'flex', overflowX: 'auto', padding: '0 16px 12px', gap: 8, scrollbarWidth: 'none', msOverflowStyle: 'none' } as React.CSSProperties}>
            {categories.map(cat => (
              <button key={cat.id} onClick={() => { setActiveCategory(cat.id); categoryRefs.current[cat.id]?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }}
          ...
          return (
            <div key={cat.id} ref={(el: HTMLDivElement | null) => { categoryRefs.current[cat.id] = el }} style={{ scrollMarginTop: 130 }}>
              <div style={{ padding: '20px 16px 10px', display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ fontWeight: 900, fontSize: 18, color: C.gold }}>{cat.name_he}</div>
              </div>
              {items.map(item => (
      ...
      {showCart && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 450 }}>
          <div onClick={() => setShowCart(false)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.75)' }} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: C.bgCard, borderRadius: '24px 24px 0 0', padding: '20px 20px 36px', direction: 'rtl', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ width: 40, height: 4, background: C.border, borderRadius: 2, margin: '0 auto 16px' }} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              <div style={{ fontWeight: 900, fontSize: 18, color: C.white }}>🛒 הסל שלך</div>
              <button onClick={() => setShowCart(false)} style={{ background: C.border, border: 'none', borderRadius: 10, width: 34, height: 34, cursor: 'pointer', fontSize: 18, color: C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Heebo, sans-serif' }}>✕</button>
            </div>
            {cart.length === 0 ? <div style={{ textAlign: 'center', color: C.gray, padding: '30px 0' }}>הסל ריק</div> : (
              <>
                {cart.map((c, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', paddingBottom: 14, marginBottom: 14, borderBottom: i < cart.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 15, color: C.white }}>{c.item.name_he}</div>
                      {c.sauces.length > 0 && <div style={{ fontSize: 12, color: C.gray, marginTop: 2 }}>רטבים: {c.sauces.join(', ')}</div>}
                      {c.salads.length > 0 && <div style={{ fontSize: 12, color: C.gray }}>סלטים: {c.salads.join(', ')}</div>}
      ...
      {showUpsell && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 500 }}>
          <div onClick={() => setShowUpsell(false)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.8)' }} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: C.bgCard, borderRadius: '24px 24px 0 0', padding: '24px 20px 36px', direction: 'rtl', maxHeight: '85vh', overflowY: 'auto' }}>
              ...
              if (drinkCat && !hasDrinks) {
                ...
                return drinks.length > 0 ? (
                  <div style={{ marginBottom: 22 }}>
                    ...
                    {drinks.map(drink => (
                      <div key={drink.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.bg, borderRadius: 14, padding: '10px 14px', border: `1px solid ${C.border}`, marginBottom: 10 }}>
                        <div style={{ width: 60, height: 60, borderRadius: 10, overflow: 'hidden', flexShrink: 0, background: C.bgCard, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {drink.image_url ? <img src={drink.image_url} alt={drink.name_he} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 28 }}>🥤</span>}
                        </div>
                        <div style={{ flex: 1 }}>
            ...
            {(() => {
              ...
              if (!hasAddons && addonItems.length > 0) return (
                <div style={{ marginBottom: 22 }}>
                  ...
                  {addonItems.map(a => (
                    <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.bg, borderRadius: 14, padding: '10px 14px', border: `1px solid ${C.border}`, marginBottom: 10 }}>
                      <div style={{ width: 60, height: 60, borderRadius: 10, overflow: 'hidden', flexShrink: 0, background: C.bgCard, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {a.image_url ? <img src={a.image_url} alt={a.name_he} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 28 }}>🍟</span>}
                      </div>
                      <div style={{ flex: 1 }}>
      ...
      {showBottomSheet && selectedItem && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 400 }}>
          <div onClick={() => setShowBottomSheet(false)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.75)' }} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: C.bgCard, borderRadius: '24px 24px 0 0', maxHeight: '92vh', overflowY: 'auto', paddingBottom: 32 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px 0' }}>
              ...
              <button onClick={() => setShowBottomSheet(false)} style={{ background: C.border, border: 'none', borderRadius: 10, width: 34, height: 34, cursor: 'pointer', fontSize: 18, color: C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>✕</button>
            </div>
            ...
            <div style={{ padding: '18px 20px' }}>
              ...
              {sauces.length > 0 && !categories.find(c => c.id === selectedItem?.category_id)?.name_he.includes('שתי') && (
                <div style={{ marginBottom: 22 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {sauces.map(s => <DarkChip key={s.id} label={s.name_he} selected={sheetSauces.includes(s.name_he)} onToggle={() => setSheetSauces(prev => prev.includes(s.name_he) ? prev.filter(x => x !== s.name_he) : [...prev, s.name_he])} />)}
                  </div>
                </div>
              )}
              ...
              {categories.find(c => c.id === selectedItem?.category_id)?.name_he.includes('עסקי') && (
                <div style={{ marginBottom: 22 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                    {SET_DRINKS_FREE.map(d => <button key={d} onClick={() => setSheetSetDrink(sheetSetDrink === d ? '' : d)} style={{ padding: '7px 14px', borderRadius: 20, border: `1px solid ${sheetSetDrink === d ? C.green : C.border}`, background: sheetSetDrink === d ? 'rgba(74,222,128,0.15)' : 'transparent', color: sheetSetDrink === d ? C.green : C.gray, fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>{d}</button>)}
                  </div>
                  <div style={{ color: C.gray, fontSize: 12, marginBottom: 8 }}>בקבוק / פיוז טי — תוספת ₪{SET_DRINK_EXTRA}</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {SET_DRINKS_PAID.map(d => <button key={d} onClick={() => setSheetSetDrink(sheetSetDrink === d ? '' : d)} style={{ padding: '7px 14px', borderRadius: 20, border: `1px solid ${sheetSetDrink === d ? C.gold : C.border}`, background: sheetSetDrink === d ? 'rgba(255,215,0,0.15)' : 'transparent', color: sheetSetDrink === d ? C.gold : C.gray, fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>{d} <span style={{ fontSize: 11 }}>+₪{SET_DRINK_EXTRA}</span></button>)}
                  </div>
                </div>
              )}
              {categories.find(c => c.id === selectedItem?.category_id)?.name_he.includes('עסקי') && (
                <div style={{ marginBottom: 22 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                    <button onClick={() => setSheetSetAddon(sheetSetAddon === SET_ADDON_FREE ? '' : SET_ADDON_FREE)} style={{ padding: '7px 14px', borderRadius: 20, border: `1px solid ${sheetSetAddon === SET_ADDON_FREE ? C.green : C.border}`, background: sheetSetAddon === SET_ADDON_FREE ? 'rgba(74,222,128,0.15)' : 'transparent', color: sheetSetAddon === SET_ADDON_FREE ? C.green : C.gray, fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>{SET_ADDON_FREE}</button>
                  </div>
                  <div style={{ color: C.gray, fontSize: 12, marginBottom: 8 }}>תוספות בתשלום</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {SET_ADDONS_PAID.map(a => <button key={a.name} onClick={() => setSheetSetAddon(sheetSetAddon === a.name ? '' : a.name)} style={{ padding: '7px 14px', borderRadius: 20, border: `1px solid ${sheetSetAddon === a.name ? C.gold : C.border}`, background: sheetSetAddon === a.name ? 'rgba(255,215,0,0.15)' : 'transparent', color: sheetSetAddon === a.name ? C.gold : C.gray, fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'Heebo, sans-serif' }}>{a.name} <span style={{ fontSize: 11 }}>+₪{a.price}</span></button>)}
                  </div>
                </div>
              )}
...
function DarkChip({ label, selected, onToggle, disabled, danger }: { label: string; selected: boolean; onToggle: () => void; disabled?: boolean; danger?: boolean }) {
